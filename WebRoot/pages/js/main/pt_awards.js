 ...
 * 添加资讯
 */
function addAwards(bodyParam){
    var httpR = new createHttpR(url+'addAwards','post','text',bodyParam,'callBack');
 ...
 * 查询单条资讯
 */
function selectAwards(id){
    var bodyParam={'id':id};
    var httpR = new createHttpR(url+'selectAwards','post','text',bodyParam,'callBack');
    httpR.HttpRequest(function(response){
        var obj = JSON.parse(response);
        var status = obj['status'];
        //var msg = obj['msg'];
 ...
 * @param id
 */
function updateAwards(bodyParam){

    var httpR = new createHttpR(url+'updateAwards','post','text',bodyParam,'callBack');
    httpR.HttpRequest(function(response){
        var obj = JSON.parse(response);
        var status = obj['status'];
        //var msg = obj['msg'];
...
function deleteAwards(id){
    var bodyParam={'id':id};
    var httpR = new createHttpR(url+'deleteAwards','post','text',bodyParam,'callBack');
    httpR.HttpRequest(function(response){
        var obj = JSON.parse(response);
        var status = obj['status'];
        //var msg = obj['msg'];
 ...
 * @param pageSize
 */
function  queryAwards (searchText,currentPage,pageSize) {
    ...
    var httpR = new createHttpR(url+'listAwards','post','text',bodyParam,'callBack');
    httpR.HttpRequest(function(response){
        var obj = JSON.parse(response);
        var status = obj['status'];
        var msg = obj['msg'];
        if(status=='0'){
            ...
            for(var o in data){
                html+='<tr index='+o+' class="gradeX">\n' +
                    ...
                    '<td style="line-height: 50px;">'+data[o].drawname+'</td>\n' +
                    '<td style="line-height: 50px;">'+data[o].rate+'</td>\n';
                if(data[o].type=='1'){
                ...
                html+='<td style="line-height: 50px;"><a class="updateAwards" href="pt_awards_detail.html?id='+data[o].id+'" index='+o+' data-toggle="modal" ><span class="label label-info label-mini">修改</span></a>   ' +
                    '<a class="deleteAwards" href="" index='+o+' data-toggle="modal" data-target="#delete-box"><span class="label label-danger label-mini">删除</span></a></td>\n';
                html+='</tr>';
            }
